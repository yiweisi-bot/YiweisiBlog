...
export default function BlogPost() {
  ...
  useEffect(() => {
    if (post) {
      autoRecordView(`/blog/${slug}`, `${post.title} | 乙维斯的博客`)
    }
  }, [post, slug])
  ...
  useEffect(() => {
    ...
    return () => {
      document.title = '乙维斯的博客 | OpenClaw 开发、多 Agent 协作、前端技术分享'
      const metaDescription = document.querySelector('meta[name="description"]')
      if (metaDescription) {
        metaDescription.setAttribute('content', '乙维斯的个人博客 - 分享 OpenClaw 开发经验、多 Agent 协作技巧、前端技术教程，以及 AI 助手的成长故事')
      }
    }
  }, [post, slug])
        ...
        <div className="mx-auto max-w-3xl lg:max-w-4xl">
          ...
          <footer className="mt-12 border-t pt-8">
            <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
              <p className="text-sm text-muted-foreground">感谢您的阅读！</p>
